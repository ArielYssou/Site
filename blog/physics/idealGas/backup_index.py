	...
	<body>
		...
		<div class="head_container">
			...
			<div class="head_middle">
				<div class="head_text"><b>The Ideal Gas</b></div>
			</div>
		</div>
		...
		<div class="container post_body">
			<div class="row post_struct">
				<div class="col-ld-12">
					...
					<p>In this project we simulate an ideal gas using <a href="https://p5js.org/" target="_blank">p5.js</a>, exploring some interesting theoretical aspects of this model like analysing the mean free path.</p>
				...
				<tr><td><a href="#THEORY">2. Theory</a>	</td></tr>
				...
				<br>
					...
					<p>
					The discoveries made at this time would lay the ground for not only the industrial revolution but to the formulation of thermodynamics itself. It was Clapayron that would first state the idal gas law in 1834 using only the empirical laws know at that time, combining several insights into one simple equation. After him Classisus (and apparently August Lrönig independently) would also derive the same equation using kinetic theory <a href="https://zenodo.org/record/1423644#.X2S7s4Z7mrw" target='_blank'>in 1857</a>. In the next chapter we will discuss its theory in greater depth.
					</p>
					<a name="THEORY"></a>
					<h2 class="section">2. Theory</h2>
					<p>
					...
					<h3>Empirical derivation</h3>
					<p>
					One can combine the empirical laws of <a href="https://en.wikipedia.org/wiki/Boyle%27s_law" target='_blank'>Boyle</a>, <a href="https://en.wikipedia.org/wiki/Charles%27s_law" target="_blank">Charles</a> and <a href="https://en.wikipedia.org/wiki/Avogadro's_law" target="_blanck">Avogadro</a> and derive the ideal gas law with simple algebra. These relations can be written as
					$$
					\begin{eqnarray}
					...
					<h3>Kinetic theory derivation</h3>
					<p>
					...
					<h3>Statistical mechanics derivation</h3>
					<p>
					...
					<h2 class="section">3. Implementation</h2>
					<p>
...
\vec{u}_2 = \vec{v}_2 - \frac{2m_1}{M}\frac{(\vec{v}_2 - \vec{v}_1)\cdot(\vec{r}_2 - \vec{r}_1)}{|\vec{r}_2 - \vec{r}_1|^2}(\vec{r}_2 - \vec{r}_1),
					\end{equation*}
					$$	
					where \(M \equiv m_1 + m_2\). A full demonstration of these equations can be found <a class="custom_link" href="https://en.wikipedia.org/wiki/Elastic_collision">here</a>, but it is a direct implication of the conservation of linear momentum and kinetic energy.
					</p>
					<p>
					Now we only need to translate these ideas into code. We want a <code>Particle</code> class such that each particle has a position, speed, radius and a mass. The position will be updated using only its speed (as there are no internal or external forces and the collisions are considered instantaneous). As we do not want any particles leaking from the container, we will also check if its position is inside the container before updating it. In python $``$pseudocode$"$:
					</p>
					<pre>
					{% include 'snippet_1.py' %}
					</pre>
					<p>
					...
					<ol>
						<li>1. Initialize the system with $N$ non-overlaping particles (before adding a new particle, check if it does not collide with any other particle. If after a number of tries you still haven't added any new particle then start only with the number that managed to get included))</li>
						<li>For each particle, check for collisions and update speeds accordingly</li>
						<li>For each particle, update the position. Goto 2.</li>
					</ol>
					<p>
						To build the collision detection method we will use a brute force approach, where we just compare each possible particle pairs. It's worth mentioning that collisions could be dealt with in a much better way using <a class="custom_link" href="https://en.wikipedia.org/wiki/Quadtree">quadtrees</a> and other fancy methods, but we will leave this to a future post.
					</p>
					<p>
					...
					<p>
					That should do it! Here is the full source code with some added functionality to make the display prettier. If there are any doubts or criticism I will be more than happy to hear then out :D Try messing with the code in the <a class="custom_link" href="https://editor.p5js.org/ArielYssou/sketches/Sqha8Ggec" target="_blank">p5js editor</a>.
					</p>
